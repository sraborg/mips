	...
	****************************************************************************/
	public synchronized void processInstruction() {
		...
		if(this.programCounter.hasNext()) {
			...
			if(currentInstruction instanceof JTypeInstruction) {
				JTypeInstruction jTypeInstruction = (JTypeInstruction) currentInstruction;

				switch(currentInstruction.getOpcode()) {
					// j
					case "000010":
						register[jTypeInstruction.gettarget()] = register[jTypeInstruction.gettarget()];
						break;
					// jal
					case "000011":
						register[jTypeInstruction.gettarget()] = register[jTypeInstruction.gettarget()];
			...
			if(currentInstruction instanceof RTypeInstruction) {
				...
				Integer rd = rTypeInstruction.getrd();
				Integer rs = rTypeInstruction.getrs();
				Integer rt = rTypeInstruction.getrt();
				Integer shift = rTypeInstruction.getshift();

				switch(rTypeInstruction.getFunction()) {
					// sll
					case "000000":
						register[rTypeInstruction.getrd()] = register[rTypeInstruction.getrt()] << rTypeInstruction.getshift();
						System.out.println(this.register[rd] + ", " + this.register[rt] +", " + String.valueOf(shift));
						this.gui.registersTableModel.setValueAt(String.format("%32s",Long.toBinaryString(Long.decode(Integer.toBinaryString(rd)))).replace(" ", "0"), rd, 1); // Set Binary Value
						this.gui.registersTableModel.setValueAt(register[rd], rd, 2); // Set Decimal Value
						this.gui.registersTable.revalidate();
						this.gui.registersTable.repaint();
						break;
					// srl
					...
					case "100000":
						...
						System.out.println(this.register[rd] + ", " + this.register[rs] +", " + this.register[rd]);
						this.gui.registersTableModel.setValueAt(String.format("%32s",Long.toBinaryString(Long.decode(Integer.toBinaryString(rd)))).replace(" ", "0"), rd, 1); // Set Binary Value
						this.gui.registersTableModel.setValueAt(register[rd], rd, 2); // Set Decimal Value
						this.gui.registersTable.revalidate();
						this.gui.registersTable.repaint();
					...
					case "100010":
						register[rTypeInstruction.getrd()] = register[rTypeInstruction.getrs()] - register[rTypeInstruction.getrt()];
						System.out.println(this.register[rd] + ", " + this.register[rs] +", " + this.register[rt]);
						this.gui.registersTableModel.setValueAt(String.format("%32s",Long.toBinaryString(Long.decode(Integer.toBinaryString(rd)))).replace(" ", "0"), rd, 1); // Set Binary Value
						this.gui.registersTableModel.setValueAt(register[rd], rd, 2); // Set Decimal Value
						this.gui.registersTable.revalidate();
						this.gui.registersTable.repaint();
	...
	public void loadFileInstructions(String file) {
    	String temp;
    	String[] tempArray;
    	String key;
    	Integer value;
